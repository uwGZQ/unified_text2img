...
# class AltDiffusion(AbstractModel):
#     def __init__(self, ckpt: str = "BAAI/AltDiffusion-m9", precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda")):
#         from diffusers import AltDiffusionPipeline
#         self.pipeline = AltDiffusionPipeline.from_pretrained(ckpt, torch_dtype=precision).to(device)
#     def text2img(self, prompt):
#         images = self.pipeline(prompt).images
#         return images[0]
class StableUnClip(AbstractModel):
    def __init__(self, ckpt: dict ={"prior_model":"kakaobrain/karlo-v1-alpha","prior_text_model":"openai/clip-vit-large-patch14","pipeline":"stabilityai/stable-diffusion-2-1-unclip-small"}, precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda")):
...
class SDXLLightning(AbstractModel):
    def __init__(self, ckpt: dict = {"repo":"ByteDance/SDXL-Lightning", "base":"stabilityai/stable-diffusion-xl-base-1.0", "step": 4, "_ckpt": "dxl_lightning_4step_unet.safetensors", "use_unet":True}, precision: torch.dtype = torch.float16, device: torch.device = torch.device("cuda")):
...
# creater = DiffusionModel()
# prompt = "a painting of a sunset over the city"
# img = creater.text2img(prompt)
# img.save("sunset1.jpg")

# # creater = DALLEModel()
# # prompt = "a painting of a sunset over the city"
# # img = creater.text2img(prompt)
# save_path = "sunset.jpg"
# img = 'https://oaidalleapiprodscus.blob.core.windows.net/private/org-0zNOqHS7vaUjKDDEPx3F3pdh/user-e0mTcI6LRWHukpZcXiWKqiZa/img-7VTNu5AFOlO6vT05pZsB6QQF.png?st=2024-03-21T06%3A00%3A39Z&se=2024-03-21T08%3A00%3A39Z&sp=r&sv=2021-08-06&sr=b&rscd=inline&rsct=image/png&skoid=6aaadede-4fb3-4698-a8f6-684d7786b067&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skt=2024-03-20T23%3A24%3A38Z&ske=2024-03-21T23%3A24%3A38Z&sks=b&skv=2021-08-06&sig=S0ag0%2BMPdGlzrHnnS8DewIqGo2sH6y0dEZXlYapEvKk%3D'
# # img open url:https://oaidalleapiprodscus.blob.core.windows.net/private/org-0zNOqHS7vaUjKDDEPx3F3pdh/user-e0mTcI6LRWHukpZcXiWKqiZa/img-7VTNu5AFOlO6vT05pZsB6QQF.png?st=2024-03-21T06%3A00%3A39Z&se=2024-03-21T08%3A00%3A39Z&sp=r&sv=2021-08-06&sr=b&rscd=inline&rsct=image/png&skoid=6aaadede-4fb3-4698-a8f6-684d7786b067&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skt=2024-03-20T23%3A24%3A38Z&ske=2024-03-21T23%3A24%3A38Z&sks=b&skv=2021-08-06&sig=S0ag0%2BMPdGlzrHnnS8DewIqGo2sH6y0dEZXlYapEvKk%3D
# import requests
# from PIL import Image
# import io
